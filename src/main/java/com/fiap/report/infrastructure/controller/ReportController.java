...
public class ReportController {
    ...
    public ResponseEntity<List<ReportSummaryResponse>> listReports(
        ...
        var reportsPage = listReportsUseCase.execute("default-user", PageRequest.of(page, size));
    ...
    private int mapProgress(com.fiap.report.domain.report.ReportStatus status) {
        ...
        switch (status) {
            ...
            case EM_PROCESSAMENTO: return 50;
            case ANALISADO: return 100;
            case ERRO: default: return 0;
        }
    }
    ...
    public ResponseEntity<byte[]> downloadReport(
            @PathVariable String uploadId,
            @RequestParam(value = "template", defaultValue = "template-tecnico") String templateId) {
        log.info("Downloading report: {} with template parameter: {}", uploadId, templateId);

        try {
            UUID diagramId = convertToUUID(uploadId);
            var report = getReportUseCase.execute(diagramId);

            String template = templateId != null ? templateId.toLowerCase() : "template-tecnico";
            String reportContent;

            if (template.contains("executivo")) {
                reportContent = generateExecutivePdf(report, uploadId);
            } else if (template.contains("seguranca") || template.contains("pci")) {
    ...
    private String generateExecutivePdf(AnalysisReport report, String uploadId) {
        ...
        return createRawPdfString("Relatorio EXECUTIVO de Arquitetura", uploadId, pdfLines.toArray(new String[0]));
    }
    ...
    private String generateSecurityPdf(AnalysisReport report, String uploadId) {
        ...
        return createRawPdfString("Auditoria de SEGURANCA e Compliance", uploadId, pdfLines.toArray(new String[0]));
    }
    ...
    private String generateTechnicalPdf(AnalysisReport report, String uploadId) {
        ...
        if (report.getComponents() != null && !report.getComponents().isEmpty()) {
            report.getComponents().forEach(comp -> pdfLines.add("- " + comp.getName() + " (" + comp.getType().name() + ")"));
        } else {
        ...
        return createRawPdfString("Analise TECNICA Detalhada (Deep Dive)", uploadId, pdfLines.toArray(new String[0]));
    }
